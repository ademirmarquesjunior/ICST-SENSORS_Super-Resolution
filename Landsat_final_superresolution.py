...
x = cv2.resize(rgb, (int(np.shape(ms_original)[1]),
                     int(np.shape(ms_original)[0])),
               interpolation=0)
...
csv_file = folder + element + '_training__mse_mae.csv'
np.savetxt(csv_file, mse_mae_history, fmt='%3.7f', 
           delimiter=';', header='MSE;MAE')

plot_mse_mae_history(element, 1, epochs, csv_file, folder)
...
for ratio in (2, 4, 8, 16):
    ...
    rgb_resized = cv2.resize(rgb, (int(np.shape(ms_original)[1]*ratio),
                         int(np.shape(ms_original)[0]*ratio)))        
        
    new_image = model.predictBands(rgb_resized/255)
    
    #Reshape predicted to the same shape as multispectral desired image
    ms_predicted_0 = np.reshape(new_image, (np.shape(rgb_resized)[0]-2, 
                                            np.shape(rgb_resized)[1]-2, 
    ...
    profile = rt.open(folder + element + "_ms.tif").profile
    filename = folder + 'composed_ratio_' + str(ratio) + '_Landsat_ms.tif'
    save_geo_image (ms_predicted, profile, filename)
    
    #rt.open(folder + "composed_Landsat_ms.tif").profile
        
    
    ms_predicted = cv2.resize(ms_predicted, (np.shape(ms_original)[1], 
    ...
    np.savetxt(folder + element + '_quality_results_ratio_'+ str(
...
np.savetxt(folder + element + '_composed_quality_results.csv', np.reshape(
    quality_composed_image, (int(np.size(
...
for  band_number in range(0,np.shape(ms_original)[2]):
    ...
    header_list.append("ratio " + str(band_number))
